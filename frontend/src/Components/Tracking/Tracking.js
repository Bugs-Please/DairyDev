...
const Tracking = () => {
  ...
  const { milkBatchNumber, sensorId, billNumber, labReportNumber } = formData;
  ...
  const FormSubmit = async (e) => {
    ...
    const fetchEvents = async () => {
      ...
      const res = await axios.get(
        'http://localhost:8081/api/getHistory',
        config
      );
    ...
    const fetchEvents2 = async () => {
      ...
      const res = await axios.get(
        'http://localhost:8082/api/getalldocs',
        config
      );
      if (res.status === 200) {
            ...
            <form onSubmit={FormSubmit}>
              <div className="row">
                <div className="col-md-6">
                  <label
                    for="milkBatchNumber"
                    className="form-label label-batch"
                  >
                    Milk Batch Number
                  </label>
                  <input
